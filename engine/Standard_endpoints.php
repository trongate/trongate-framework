...
class Standard_endpoints extends Trongate
{
    ...
    public function get($return_row_count = false): void //GET
    ...
    public function search($return_row_count = false): void //POST
    {
        $request_type = $this->get_request_type();
    ...
    public function find_one(): void //GET
    {
        $request_type = $this->get_request_type();
        if ($request_type !== 'GET') {
            http_response_code(400);
    ...
    public function count(): void //GET or POST
    {
        $request_type = $this->get_request_type();
        if ($request_type !== 'GET') {
            $this->search(true);
    ...
    public function make_sure_columns_exist($table_name, $params, $valid_columns = null)
    {
        ...
        if (count($invalid_columns) === 1) {
            return "The {$invalid_columns[0]} column does not exist on the {$table_name} table!";
        }
        return "The following columns do not exist on the {$table_name} table: ".implode(', ', $invalid_columns);
    }

    public function insert(): void // BATCH INSERT!
    {
        $request_type = $this->get_request_type();
        if ($request_type !== 'POST') {
            http_response_code(400);
    ...
    {
        $allowed_request_types = segment(1) === 'api' ? ['POST', 'PUT'] : ['PUT'];
        $target_segment = segment(1) === 'api' ? 4 : 2;
        $update_id = intval(segment($target_segment));
    ...
    {
        $allowed_request_types = segment(1) === 'api' ? ['POST', 'DELETE'] : ['DELETE'];
        $request_type = $this->get_request_type();
    ...
    public function attempt_serve_standard_endpoint($endpoint_index): void
    {
    ...
    public function reduce_query_params($query_params)
    {
    ...
    {
        return Modules::run($table_name.'/'.$before_hook, $input);
    }

    private function attempt_invoke_after_hook($table_name, $after_hook, $output)
    {
        return Modules::run($table_name.'/'.$after_hook, $output);
    }
    ...
    private function test_for_aaa_token($token)
    {
        ...
        $code = $token_obj->code ?? '';
        return $code === 'aaa' ? true : false;
    }

    private function run_user_owned_test($test_data): void
    {
    ...
    private function api_manager_error($response_status_code, $error_msg): void
    {
