...
function tgpBuildNavBackBtn(btnContainer) {
    ...
    navBackBtn.addEventListener('click', (ev) => {
        trongatePagesObj.currentImgDir = tgpRemoveLastSegment(trongatePagesObj.currentImgDir);
...
function tgpDrawAddNewFolderForm(parentContainer) {
    ...
    input.addEventListener('input', function() { // Add event listener for input event
        const value = input.value; // Get the current value of the input field

        if (!safeChars.test(value)) { // Test if the value contains any unsafe characters
            ...
            alert('Only letters, numbers, hyphens, and underscores are allowed for folder names.'); // Alert the user
        }
    });

    input.addEventListener('paste', function(event) { // Add event listener for paste event
...
function tgpAttemptCreateNewFolder() {
    ...
    const targetUrl = trongatePagesObj.baseUrl + 'trongate_pages/submit_create_new_img_folder';
...
function tgpInitConfDeleteFolder(clickedBtn, targetModalBody) {
    ...
    const realFolderNameEl = targetPreviewBox.querySelector('.tgp-real-entity-name');

    const directoryInfoPara = document.createElement('p');
    directoryInfoPara.innerHTML = 'Folder Name:  <span class="tgp-grey-label">' + realFolderNameEl.innerHTML + '</span>';
...
function tgpExecuteDeleteFolder(folderName) {
    const tgpDeleteImgCodeInput = document.getElementById('tgp-delete-folder-code-input');
...
function tgpSendDeleteFolderRequest(folderName) {
    ...
    const targetUrl = trongatePagesObj.baseUrl + 'trongate_pages/submit_delete_folder';
    const http = new XMLHttpRequest();
...
function tgpRestoreElement(previewBox, attemptRenameFolder = true) {
    if (previewBox.classList.contains('tgp-preview-box-edit-mode')) {
        ...
        const elementNamePreviewDiv = previewBox.querySelector('.tgp-preview-element-name');
...
function tgpAttemptRenameFolder(previewBox, oldFolderName, newFolderName) {
    ...
    for (var i = tgpPreviewBoxes.length - 1; i >= 0; i--) {
        if (!tgpPreviewBoxes[i].classList.contains('tgp-preview-box-edit-mode')) {
            const realEntityNameEl = tgpPreviewBoxes[i].querySelector('.tgp-real-entity-name');
            if (realEntityNameEl) {
...
function tgpSubmitRequestRenameFolder(previewBox, oldFolderName, newFolderName) {
    ...
    const targetUrl = trongatePagesObj.baseUrl + 'trongate_pages/submit_rename_img_folder';
    const http = new XMLHttpRequest();
    http.open('post', targetUrl);
    http.setRequestHeader('Content-type', 'application/json');
    http.setRequestHeader('trongateToken', trongatePagesObj.trongatePagesToken);
...
function tgpRestorePreviewBox(previewBox, realFolderName) {
    ...
    const imgPath = trongatePagesObj.baseUrl + 'trongate_pages' + trongatePagesObj.moduleAssetsTrigger + '/images/foldericon.png';
...
document.addEventListener('click', (ev) => {
    const tgpPreviewBoxesInEditMode = document.getElementsByClassName('tgp-preview-box-edit-mode');
    if (tgpPreviewBoxesInEditMode.length > 0) {
        const clickedEl = ev.target;
        const containingPreviewBoxInEditMode = clickedEl.closest('.tgp-preview-box-edit-mode');
