...
function tgpDrawNavFolderIcon(folderName, uploadedImagesGrid) {
    ...
    gridBox.addEventListener('dblclick', (ev) => {
        ...
        trongatePagesObj.currentImgDir = trongatePagesObj.currentImgDir + '/' + dirName;
...
function tgpSpinBabySpin() {
    ...
    const allUploadedPicIcons = document.querySelectorAll('#tgp-media-manager > div.modal-body .tgp-preview-box img');
    for (var i = 0; i < allUploadedPicIcons.length; i++) {
        ...
        if (tgpIconTypeToSpin === 'image') {
            ...
            const realFolderNameEl = thisPreviewBox.querySelector('.tgp-real-entity-name');
...
function tgpInitConfDeletePic(ev) {
    const tgpPicBtnsPara = document.querySelector('#tgp-media-manager > div.modal-body > p:nth-child(3)');
    const targetModalBody = document.querySelector('#tgp-media-manager > div.modal-body');
...
function tgpExecuteDeleteImg() {
    const tgpDeleteImgCodeInput = document.getElementById('tgp-delete-img-code-input');
