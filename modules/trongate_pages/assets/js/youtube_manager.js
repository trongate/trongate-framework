function tgpBuildInsertYouTubeVideoModal() {
    ...
    modalHeading.innerHTML = '<i class="fa fa-youtube-play"></i> Add YouTube Video';
    ...
    infoParaText = document.createTextNode('Enter the ID or URL of the target YouTube video then hit `Submit`.');
...
function tgpSubmitVideoId() {
  ...
  const modalBody = document.querySelector('#tgp-youtube-modal > div.modal-body');
  ...
  const targetUrl = trongatePagesObj.baseUrl + 'trongate_pages/check_youtube_video_id';
  ...
  http.onload = function() {
    ...
    if(http.status === 200) {
      ...
      tgpProcValidationErr(modalBody, http.status, 'You did not submit a valid YouTube video ID');
    }
...
function tgpBuildConfirmVideo(modalBody, youTubeVideoId) {
  const modalHeadline = document.querySelector('#tgp-youtube-modal > div.modal-body > p');
  modalHeadline.innerHTML = 'Does this look okay?';
  modalHeadline.classList.remove('blink');

  const spinnerEl = document.querySelector('#tgp-youtube-modal > div.modal-body > div');
  spinnerEl.remove();
  ...
  confirmBtnYes.setAttribute('onclick', 'tgpDestroyModalBuildVideo("' + youTubeVideoId + '")');
...
function tgpBuildYouTubeVideoPlayer(parentEl, youTubeVideoId) {
  ...
  newIframe.setAttribute('src', 'https://www.youtube.com/embed/' + youTubeVideoId);
...
function tgpDestroyModalBuildVideo(youTubeVideoId) {
  tgpReset(['selectedRange', 'codeviews', 'customModals', 'toolbars', 'activeEl']);
  tgpBuildYouTubeVideoPlayer(trongatePagesObj.defaultActiveElParent, youTubeVideoId)
}
...
function tgpAddOverlayToYoutubeVideos() {
  ...
  parentElement.addEventListener('mouseover', (event) => {
    ...
    if (!isInsideModalBody && (event.target.matches('.video-container') || event.target.matches('.video'))) {
...
function tgpAddVideoToolbar(targetEl) {
  ...
  youtubeRedirectBtn.innerHTML = "Watch On YouTube <i class='fa fa-youtube-play'></i>";
