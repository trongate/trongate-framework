function tgpDeletePage() {
  ...
  modalForm.setAttribute('action', trongatePagesObj.baseUrl + 'trongate_pages/submit_delete/' + trongatePagesObj.trongatePagesId);
...
function tgpTrashify() {
    ...
    const infoParaText = document.createTextNode('Are you sure you want to remove the selected element?');
    ...
    closeBtn.setAttribute('onclick', 'tgpCloseAndDestroyModal(\'' + modalId + '\', false)');
...
function tgpFetchSettings() {
    const targetUrl = trongatePagesObj.baseUrl + 'api/get/trongate_pages/' + trongatePagesObj.trongatePagesId;
    ...
    http.onload = (ev) => {
      ...
      if(http.status === 200) {
        ...
        const spinnerEl = document.querySelector('#tgp-settings-modal > div.modal-body > div');
...
function tgpCreateSettingsForm(pageSettings={}) {
  ...
  metaKeywordsInput.value = pageSettings.meta_keywords ? pageSettings.meta_keywords : '';
  ...
  metaDescriptionTextarea.value = pageSettings.meta_description ? pageSettings.meta_description : '';
  ...
  const modalBody = document.querySelector('#tgp-settings-modal > div.modal-body');

  // Append the form to the document body or another container element
  modalBody.appendChild(form);

  const submitBtn = document.querySelector('#tgp-settings-modal > div.modal-footer > button:nth-child(2)');
...
function tgpInterceptAddPageElement(el, newElType) {
    ...
    let infoPara = document.createElement('p');
    infoParaText = document.createTextNode('Where would you like to add the new ' + newElType + '?');
    infoPara.appendChild(infoParaText);
