    ...
    <ul class="options-selector">
        ...
        if (!empty($properties)):
            foreach ($properties as $prop):
                ...
                if ($prop_name !== ''):
                    $order_by_options[] = ['propertyName' => $prop_name];
                    $order_by_options[] = ['propertyName' => $prop_name . ' DESC'];
                endif;
            endforeach;
        endif;
        foreach ($order_by_options as $opt):
            $opt_name = $opt['propertyName'];
            $mx_vals = json_encode(['selected' => $opt_name], JSON_HEX_APOS | JSON_UNESCAPED_UNICODE);
        ?>
